...
def validar_rubro():
    ...
    while rubro != "i" and rubro != "p" and rubro != "c":
        rubro = input(
            "error, ingrese un rubro valido (indumentaria (i) / perfumeria (p) / comida (c)): "
        )
    return rubro
...
def inicio():
    [email,password,tipo] = validar_ingresoo()

    validar_menu() if validar_ingreso() else print(
        "Alcanzó el límite de inicio de sesion"
    )
